'''FEAT-20260923-001 采购单按供应商分类展示与筛选：后端接口验收冒烟（独立外部验收）。

采购单与供应商接口沿用已验收的契约（/biz/purchaseOrder、/biz/supplier）。本次新增的供应商分类展示字段和
筛选参数不绑定实现：展示字段按回读的值识别（字典名称；或者是字典值，再按字典翻译），筛选参数从候选别名中识别，
『未分类』条件从前端源码和常见取值中识别。
...
UNCATEGORIZED = '未分类'
LABELS = ('原材料', '服务', '设备')
OTHER_DEPT_ID = '1761000000000000105'  # 测试部门；admin 属于研发部门
BIG_PAGE = 200
...
class World:
    ...
    def user_without(self, action: str) -> Client:
        '''A user in another department granted every purchase-order button except `action`.'''
        menus = self.admin.get('/system/menu/list')['data'] or []
        top = next((m for m in menus if m.get('menuName') == DIR_NAME and str(m.get('parentId')) == '0'), None)
        if top is None:
            raise AssertionError('菜单中没有顶级目录『业务管理』')
        page = next((m for m in menus if m.get('menuName') == MENU_NAME
                     and str(m.get('parentId')) == str(top['menuId'])), None)
        if page is None:
            raise AssertionError('『业务管理』下没有『采购单管理』菜单')
        buttons = [m for m in menus if str(m.get('parentId')) == str(page['menuId'])]
        menu_ids = [top['menuId'], page['menuId']]
        menu_ids += [b['menuId'] for b in buttons if str(b.get('perms') or '').split(':')[-1] != action]
        t = tok(8)
        role_key = 'poc_' + t
        self.admin.post('/system/role', {
            'roleName': '采购分类' + t, 'roleKey': role_key, 'roleSort': 99, 'status': '0', 'dataScope': '3',
            'menuCheckStrictly': True, 'deptCheckStrictly': True, 'menuIds': menu_ids})
        roles = page_of(self.admin.get('/system/role/list',
                                       {'roleKey': role_key, 'pageNum': 1, 'pageSize': 10}))['rows'] or []
        role_id = next((r['roleId'] for r in roles if r.get('roleKey') == role_key), None)
        if role_id is None:
            raise AssertionError(f'创建的测试角色 {role_key} 查询不到')
        username, password = 'poc' + t, 'Smoke#' + t
        self.admin.post('/system/user', {
            'userName': username, 'nickName': '采购分类' + t, 'password': password,
...
class QueryTest(Case):
    ...
    def test_A2_filter_by_order_date_range(self):
        w = self.w
        supplier = w.supplier('设备', name='冒烟采购日期' + tok())
        inside = w.order(supplier, order_date=day())
        outside = w.order(supplier, order_date=day(-10))
        begin, end = day(-3), day()
        rows, _ = w.rows(supplierName=supplier['supplierName'],
                         extra={'params[beginOrderDate]': begin, 'params[endOrderDate]': end})
        ids = {str(r.get('orderId')) for r in rows}
        self.assertIn(str(inside['orderId']), ids, '范围内（含结束当天）的单据没有返回')
        self.assertNotIn(str(outside['orderId']), ids, '范围外的单据也被返回了')
        for row in rows:
            self.assertTrue(begin <= str(row.get('orderDate'))[:10] <= end, row)
...
class CategoryDisplayTest(Case):
    def test_A36_row_shows_supplier_category_label(self):
    ...
    def test_A37_empty_supplier_category_shows_uncategorized(self):
        rows, _ = self.w.fixture_rows()
        self.assert_category(rows, 'empty', UNCATEGORIZED)

    def test_A38_invalid_supplier_category_shows_uncategorized(self):
        rows, _ = self.w.fixture_rows()
        self.assert_category(rows, 'invalid', UNCATEGORIZED)

    def test_A44_deleted_supplier_keeps_category(self):
        rows, _ = self.w.fixture_rows()
...
class CategoryFilterTest(Case):
    def test_A39_filter_by_service_category(self):
        w = self.w
    ...
    def test_A40_filter_uncategorized(self):
        w = self.w
    ...
    def test_A41_changed_supplier_category_is_used(self):
        w = self.w
    ...
    def test_A42_filter_by_category_and_submitted_status(self):
        w = self.w
        params = w.by_category('服务')
        rows, _ = w.fixture_rows(extra=params, status=w.submitted)
        self.assertEqual(set(rows), {w.ids['service_submitted']}, rows)
        all_rows, _ = w.rows(extra=params, status=w.submitted)
        self.assertTrue(all_rows)
        for row in all_rows:
            self.assertEqual(w.display(row), '服务', row)
            self.assertEqual(str(row.get('status')), w.submitted, row)

    def test_A47_deleted_supplier_order_matches_its_category(self):
        w = self.w
    ...
    def test_A48_deleted_supplier_order_not_uncategorized(self):
        w = self.w
        rows, _ = w.fixture_rows(extra=w.uncategorized())
        self.assertIn(w.ids['empty'], rows)
...
class ExportTest(Case):
    ...
    def test_A26_export_requires_permission(self):
        w = self.w
        user = w.user_without('export')
        ctype, content = w.export(client=user)
    ...
    def test_A45_export_by_service_category(self):
        w = self.w
        params = w.by_category('服务')
    ...
    def test_A46_export_empty_category_as_uncategorized(self):
        w = self.w
