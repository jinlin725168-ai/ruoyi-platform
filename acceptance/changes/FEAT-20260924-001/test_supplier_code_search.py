"""FEAT-20260924-001 供应商编码搜索：模糊匹配、忽略首尾空白字符和英文大小写。后端接口验收冒烟（独立外部验收）。

核心链路：列表和导出新增编码条件，与名称条件相同：编码包含输入内容即命中，先去掉首尾空白字符（半角空格、全角空格、
制表符、换行），忽略英文字母大小写，中间的空白字符按原样参与匹配；去掉首尾空白后为空时视同没有填写编码条件。
不能坏的基础功能沿用 FEAT-20260923-002 已验收的供应商列表、名称规则、分类筛选、『未分类』和导出用例，以及它们的数据准备方式：
...
# 查询条件里的空白字符：半角空格、全角空格（U+3000）、制表符、换行
...
class SupplierCase(unittest.TestCase):
    ...
    def mixed_code(self, prefix: str = "") -> str:
        """A unique supplier code containing mixed-case English letters."""
        code = prefix + "Ab" + tok(8).upper() + "Cd"
    ...
    def assert_blank_is_no_condition(self, logical: str, blank: str):
        s = self.s
        self.new(status="0")
        self.new(status="1", category=self.cat("服务"))
        _, total = s.rows(size=DEFAULT_PAGE)
        rows, blank_total = s.rows(size=DEFAULT_PAGE, **{logical: blank})
        self.assertGreaterEqual(total, 2)
        self.assertEqual(blank_total, total, f"{logical} 条件 {blank!r} 的总条数与不带条件不同")
...
class NameRuleTest(SupplierCase):
    ...
    def test_A66_only_half_width_spaces_is_no_condition(self):
        self.assert_blank_is_no_condition("name", SPACE * 4)

    def test_A71_only_mixed_whitespace_is_no_condition(self):
        self.assert_blank_is_no_condition("name", BLANK_MIX)


class CodeRuleTest(SupplierCase):
    def assert_code_hits(self, query, wanted, other=None, fragment=None):
        """List by code `query`: `wanted` is found, `other` is not, every row's code contains `fragment` ignoring case."""
        s = self.s
        rows, _ = s.rows(code=query)
        codes = s.codes(rows)
        self.assertIn(s.value(wanted, "code"), codes, f"编码条件 {query!r} 没有查到 {s.value(wanted, 'code')}: {rows}")
        if other is not None:
            self.assertNotIn(s.value(other, "code"), codes, f"编码条件 {query!r} 查到了不匹配的 {s.value(other, 'code')}")
        if fragment is not None:
            for row in rows:
                self.assertIn(fragment.lower(), str(s.value(row, "code")).lower(), row)

    def pair(self, prefix: str):
        code = self.mixed_code(prefix)
        return code, self.new(code=code), self.new(code=prefix + "Z" + tok())

    def test_A82_filter_by_code_fragment(self):
        middle = tok(8).upper()
        wanted = self.new(code="SUP-" + middle + "-01")
        other = self.new(code="SUP-" + tok(8).upper() + "-02")
        self.assert_code_hits(middle, wanted, other, middle)

    def test_A73_code_leading_trailing_half_width_spaces_ignored(self):
        code, wanted, other = self.pair("KH")
        self.assert_code_hits(SPACE * 3 + code + SPACE * 2, wanted, other, code)

    def test_A74_code_full_width_space_tab_newline_ignored(self):
        code, wanted, other = self.pair("KF")
        self.assert_code_hits(FULL + TAB + code + LF, wanted, other, code)

    def test_A75_code_lower_case_condition_matches(self):
        code = self.mixed_code("KL")
        wanted = self.new(code=code)
        self.assert_code_hits(code.lower(), wanted, fragment=code)

    def test_A76_code_upper_case_with_spaces_matches(self):
        code = self.mixed_code("KU")
        wanted = self.new(code=code)
        query = SPACE * 2 + code.upper() + SPACE
        self.assertNotEqual(query.strip(), code)
        self.assert_code_hits(query, wanted, fragment=code)

    def test_A83_code_fragment_with_changed_case_and_spaces(self):
        fragment = "Ab" + tok(8).upper() + "Cd"
        wanted = self.new(code="HB-" + fragment + "-9")
        other = self.new(code="HN-" + tok(8).upper() + "-9")
        query = SPACE * 2 + fragment.swapcase() + SPACE * 2
        self.assertNotIn(query.strip(), fragment)
        self.assert_code_hits(query, wanted, other, fragment)

    def test_A77_code_inner_space_is_kept(self):
        t = tok(6)
        spaced = self.new(code="Ab 01-" + t)
        joined = self.new(code="Ab01-" + t)
        self.assert_code_hits("ab 01", spaced, joined, "ab 01")

    def test_A78_code_only_mixed_whitespace_is_no_condition(self):
        self.assert_blank_is_no_condition("code", BLANK_MIX)

    def test_A79_code_and_name_combined(self):
        s = self.s
        name = "编码名称组合" + tok()
        code = self.mixed_code("KC")
        wanted = self.new(code=code, name=name)
        self.new(code="KC" + "Z" + tok(), name=name)
        rows, _ = s.rows(code=code.swapcase(), name=name)
        self.assertEqual(s.codes(rows), {s.value(wanted, "code")}, rows)
...
class ExportTest(SupplierCase):
    ...
    def test_A21_export_values_match_system(self):
        s = self.s
        t = tok()
        enabled = self.new(name="导出核对启用" + t, status="0", category=self.cat("设备"))
        disabled = self.new(name="导出核对停用" + t, status="1", category=self.cat("服务"))
        header, data = s.export(name=t)
        self.assertEqual(len(data), 2, data)
        for row, label, status in ((enabled, "设备", "正常"), (disabled, "服务", "停用")):
            line = self.export_row(header, data, s.value(row, "code"))
            expected = {"名称": s.value(row, "name"), "分类": label, "联系人": s.value(row, "contact"),
                        "电话": s.value(row, "phone"), "状态": status, "备注": s.value(row, "remark")}
            for title, value in expected.items():
                self.assertEqual(cell_of(line, column(header, title)), str(value), f"导出的『{title}』与系统不一致: {line}")
    ...
    def assert_export_matches_list(self, logical: str, query: str, wanted: dict, other: dict | None = None):
        """Export by `logical` = `query`: it contains `wanted`, not `other`, as many rows as the list total."""
        s = self.s
        _, total = s.rows(size=DEFAULT_PAGE, **{logical: query})
        header, data = s.export(**{logical: query})
        self.export_row(header, data, s.value(wanted, "code"))
        if other is not None:
            index = column(header, "编码")
            self.assertNotIn(s.value(other, "code"), {cell_of(r, index) for r in data})
        self.assertEqual(len(data), total, f"{logical} 条件 {query!r} 的导出行数与列表总条数不同")

    def test_A67_export_name_with_changed_case_and_spaces(self):
        name = self.mixed("导出大小写")
        wanted = self.new(name=name)
        self.assert_export_matches_list("name", SPACE * 2 + name.swapcase() + SPACE * 3, wanted)

    def test_A72_export_name_with_full_width_space_tab_newline(self):
        name = self.mixed("导出空白")
        wanted = self.new(name=name)
        self.assert_export_matches_list("name", FULL + TAB + name + LF, wanted)

    def assert_blank_export_with_disabled(self, logical: str):
        s = self.s
        self.new(status="0")
        self.new(status="1")
        _, total = s.rows(status="1", size=DEFAULT_PAGE)
        self.assertGreaterEqual(total, 1)
        header, data = s.export(status="1", **{logical: BLANK_MIX})
        index = column(header, "状态")
        self.assertEqual(len(data), total)
        for row in data:
            self.assertEqual(cell_of(row, index), "停用", row)
    ...
    def test_A68_export_blank_name_with_disabled_status(self):
        self.assert_blank_export_with_disabled("name")

    def test_A84_export_by_code_fragment(self):
        middle = tok(8).upper()
        wanted = self.new(code="EXP-" + middle + "-1")
        other = self.new(code="EXP-" + tok(8).upper() + "-2")
        self.assert_export_matches_list("code", middle, wanted, other)

    def test_A80_export_code_with_changed_case_full_width_space_tab_newline(self):
        code = self.mixed_code("EX")
        wanted = self.new(code=code)
        self.assert_export_matches_list("code", FULL + TAB + code.swapcase() + LF, wanted)

    def test_A81_export_blank_code_with_disabled_status(self):
        self.assert_blank_export_with_disabled("code")
