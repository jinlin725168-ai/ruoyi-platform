// FEAT-20260924-001 供应商编码搜索：前端核心流程与不能坏的基础功能冒烟（A8 A20 A43 A44 A52 A56 A59）。
// 登录、菜单、查询区和表格的定位器与文案沿用 acceptance/ui/supplier-management 和已验收的 FEAT-20260923-002 冒烟。
...
test('A20 点击导出下载 Excel 文件', async ({ page }) => {
  const t = uid();
  await createWithLabel(`GX${t}`, `编码导出${t}`, '设备');
  await openSupplierPage(page);
