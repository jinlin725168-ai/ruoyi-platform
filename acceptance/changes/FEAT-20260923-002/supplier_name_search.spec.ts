// FEAT-20260923-002 供应商名称搜索忽略首尾空白字符和英文大小写：前端核心流程冒烟（A8 A20 A43 A44 A52 A56 A59）。
// 登录、菜单、查询区和表格的定位器与文案沿用 acceptance/ui/supplier-management 和已验收的 FEAT-20260921-001 冒烟。
...
test('A8 按名称搜索只显示符合条件的供应商并显示分页，名称首尾空格和大小写不影响结果', async ({ page }) => {
  const t = uid();
  const hit = `名称搜索命中${t}`;
  await createWithLabel(`GA${t}`, hit, '服务');
  await createWithLabel(`GB${t}`, `名称搜索未中${t}`, '服务');
  ...
  const padded = `  ${hit.toUpperCase()}  `;
  expect(padded.trim()).not.toBe(hit);
  await search(page, { name: padded });
  await expect(tableRows(page)).toHaveCount(1);
  await expect(tableRows(page).first()).toContainText(hit);
  await expect(page.locator('.el-pagination').first()).toBeVisible();
});
...
test('A20 点击导出下载 Excel 文件', async ({ page }) => {
  const t = uid();
  await createWithLabel(`GX${t}`, `名称导出${t}`, '设备');
  await openSupplierPage(page);
